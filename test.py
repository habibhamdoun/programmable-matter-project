...
class InteractiveGrid:
    ...
    def prioritize_targets(self, targets):
        """Prioritize target positions based on the selected shape with HIGH PRIORITY for inside-out filling"""
        # Calculate center for all shapes
        center_row = self.grid_size // 2 - 1
        center_col = self.grid_size // 2 - 1

        # HIGH PRIORITY: For all shapes, prioritize filling from the inside out
        # This is the primary change to ensure inside-out filling for all shapes
        if self.selected_shape.get() == "rectangle":
            # Sort by Manhattan distance from center (inside-out)
            return sorted(targets, key=lambda pos: abs(pos[0] - center_row) + abs(pos[1] - center_col))

        elif self.selected_shape.get() == "pyramid":
            # For pyramid, prioritize by row (top to bottom) and then by distance from center column
            def pyramid_priority(pos):
                # First by row (top rows first)
                row_priority = pos[0] * 100
                # Then by distance from center column (center first)
            ...
            # For these shapes, prioritize center positions first
            # Calculate distance from center and use as primary sort key
            return sorted(targets, key=lambda pos: abs(pos[0] - center_row) + abs(pos[1] - center_col))

        elif self.selected_shape.get() == "circle":
            # For circle, prioritize positions closest to the center first
            # Use Euclidean distance for more accurate circle filling
            return sorted(targets, key=lambda pos: ((pos[0] - center_row)**2 + (pos[1] - center_col)**2)**0.5)

        elif self.selected_shape.get() == "cross":
            # For cross, prioritize center positions first, then the vertical line
            def cross_priority(pos):
                # Distance from center (lower is higher priority)
                center_dist = abs(pos[0] - center_row) + abs(pos[1] - center_col)
                # Vertical line has higher priority
            ...
            # For custom shapes, prioritize by distance from center
            # This ensures inside-out filling regardless of the custom shape
            return sorted(targets, key=lambda pos: abs(pos[0] - center_row) + abs(pos[1] - center_col))

        # Default fallback - sort by distance from center
        return sorted(targets, key=lambda pos: abs(pos[0] - center_row) + abs(pos[1] - center_col))
