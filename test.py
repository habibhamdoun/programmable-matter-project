...
class InteractiveGrid:
    def __init__(self, root, grid_size=DEFAULT_GRID_SIZE, cell_size=DEFAULT_CELL_SIZE, num_active_cells=DEFAULT_NUM_ACTIVE_CELLS):
        ...
        self.status_var = tk.StringVar(
            value="Ready. Select options and press 'Form Shape'")
        self.counter_var = tk.StringVar(
            value=f"Active cells: {num_active_cells}/{num_active_cells}")
        self.metrics_var = tk.StringVar(
            value="Steps: 0 | Time: 0.0s | Cost: 0.0")
    ...
    def setup_ui(self):
        ...
        self.canvas = tk.Canvas(self.canvas_frame,
                                width=min(800, self.grid_size *
                                          self.cell_size),
                                height=min(600, self.grid_size *
                                           self.cell_size),
                                bg="lightgray",
        ...
        tk.Label(config_frame, text="Grid Size:").grid(
            row=0, column=0, padx=5, pady=2)
        self.grid_size_var = tk.IntVar(value=self.grid_size)
        self.grid_size_entry = tk.Entry(
            config_frame, textvariable=self.grid_size_var, width=5)
        self.grid_size_entry.grid(row=0, column=1, padx=5, pady=2)

        tk.Label(config_frame, text="Cell Size:").grid(
            row=0, column=2, padx=5, pady=2)
        self.cell_size_var = tk.IntVar(value=self.cell_size)
        self.cell_size_entry = tk.Entry(
            config_frame, textvariable=self.cell_size_var, width=5)
        self.cell_size_entry.grid(row=0, column=3, padx=5, pady=2)

        tk.Label(config_frame, text="Active Cells:").grid(
            row=0, column=4, padx=5, pady=2)
        self.num_cells_var = tk.IntVar(value=self.num_active_cells)
        self.num_cells_entry = tk.Entry(
            config_frame, textvariable=self.num_cells_var, width=5)
        self.num_cells_entry.grid(row=0, column=5, padx=5, pady=2)

        self.apply_config_btn = tk.Button(
            config_frame, text="Apply Configuration", command=self.apply_configuration)
        ...
        self.design_shape_btn = tk.Button(
            shape_frame, text="Design Custom Shape", command=self.open_shape_designer)
        ...
        self.obstacle_btn = tk.Button(
            obstacle_frame, text="Place Obstacles", command=self.toggle_obstacle_mode)
        self.obstacle_btn.pack(pady=2)

        self.random_obstacles_btn = tk.Button(
            obstacle_frame, text="Random Obstacles", command=self.generate_random_obstacles)
        self.random_obstacles_btn.pack(pady=2)

        self.clear_obstacles_btn = tk.Button(
            obstacle_frame, text="Clear Obstacles", command=self.clear_obstacles)
        ...
        info_btn = tk.Button(coord_frame, text="?", width=2,
                             command=self.show_coordination_info)
        ...
        algorithm_frame = tk.LabelFrame(
            control_frame, text="Pathfinding Algorithm")
        ...
        algo_info_btn = tk.Button(
            algorithm_frame, text="Algorithm Info", command=self.show_algorithm_details)
        ...
        self.heuristic_label = tk.Label(
            heuristic_frame, text="Balanced (Default)")
        ...
        self.form_button = tk.Button(
            button_frame, text="Form Shape", command=self.form_shape)
        self.form_button.pack(pady=5)

        self.reset_button = tk.Button(
            button_frame, text="Animated Reset", command=self.reset_grid)
        self.reset_button.pack(pady=5)

        self.quick_reset_button = tk.Button(
            button_frame, text="Quick Reset", command=self.quick_reset_grid)
        self.quick_reset_button.pack(pady=5)

        self.counter_label = tk.Label(
            button_frame, textvariable=self.counter_var)
    ...
    def toggle_obstacle_mode(self):
        ...
        if self.obstacle_mode:
            ...
            self.status_var.set(
                "Obstacle placement mode: Click on grid cells to add/remove obstacles")
        else:
    ...
    def show_help(self):
        ...
        general_text_widget = scrolledtext.ScrolledText(
            general_frame, wrap=tk.WORD, width=60, height=20)
        ...
        algo_text_widget = scrolledtext.ScrolledText(
            algo_frame, wrap=tk.WORD, width=60, height=20)
        ...
        coord_text_widget = scrolledtext.ScrolledText(
            coord_frame, wrap=tk.WORD, width=60, height=20)
        ...
        movement_text_widget = scrolledtext.ScrolledText(
            movement_frame, wrap=tk.WORD, width=60, height=20)
    ...
    def show_algorithm_info(self):
        """Switch pathfinding algorithm without showing information"""
        # This method is now just a placeholder for the radio button command
        # It doesn't display any algorithm information when switching algorithms
        pass

    def show_algorithm_details(self):
        """Show information about the currently selected algorithm when button is clicked"""
        ...
        text_widget = scrolledtext.ScrolledText(
            self.algorithm_info_window, wrap=tk.WORD, width=50, height=20)
        text_widget.insert(tk.END, info.get(
            algorithm, "No information available"))
        ...
        x = (screen_width - window_width) // 2
        y = (screen_height - window_height) // 2
        self.algorithm_info_window.geometry(
    ...
    def show_coordination_info(self):
        ...
        text_widget = scrolledtext.ScrolledText(
            self.coordination_info_window, wrap=tk.WORD, width=50, height=20)
        ...
        x = (screen_width - window_width) // 2
        y = (screen_height - window_height) // 2
        self.coordination_info_window.geometry(
            f"{window_width}x{window_height}+{x}+{y}")
    ...
    def toggle_diagonal_movement(self):
        ...
        if ALLOW_DIAGONAL:
            self.status_var.set(
                "Diagonal movement enabled - cells can move in 8 directions")
        else:
            self.status_var.set(
                "Diagonal movement disabled - cells can only move in 4 directions")
    ...
    def toggle_coordination_mode(self):
        ...
        if COORDINATION_MODE:
            self.status_var.set(
                "Centralized coordination enabled - cells will work together to resolve deadlocks")
        else:
            self.status_var.set(
                "Distributed intelligence mode - cells make decisions independently")
    ...
    def toggle_cohesion_mode(self):
        ...
        if COHESION_MODE:
            self.status_var.set(
                "Cohesion mode enabled - cells will stay connected as a group")
        else:
            self.status_var.set(
                "Cohesion mode disabled - cells can move independently")
    ...
    def apply_configuration(self):
        try:
            ...
            new_num_cells = max(
                4, min(new_grid_size * new_grid_size // 2, self.num_cells_var.get()))
            ...
            self.status_var.set(
                f"Grid resized to {self.grid_size}x{self.grid_size} with {self.num_active_cells} active cells")
            self.counter_var.set(
                f"Active cells: {self.num_active_cells}/{self.num_active_cells}")
    ...
    def update_metrics_display(self):
        ...
        self.metrics_var.set(
            f"Steps: {self.step_count} | Time: {elapsed_time:.1f}s | Cost: {self.total_cost:.1f}")
    ...
    def update_target_highlights(self):
        ...
        for pos in self.cells:
            if not self.cells[pos]["active"] and pos not in self.obstacles:
                self.canvas.itemconfig(
                    self.cells[pos]["rect"], fill=INACTIVE_COLOR)
        ...
        for pos in target_positions:
            if pos in self.cells and not self.cells[pos]["active"] and pos not in self.obstacles:
                self.canvas.itemconfig(
                    self.cells[pos]["rect"], fill=TARGET_HIGHLIGHT_COLOR)
    ...
    def get_target_positions(self):
        ...
        elif self.selected_shape.get() == "exact_pyramid":
            return self.get_exact_pyramid_shape()
    ...
    def get_pyramid_shape(self):
        """Generate pyramid shape positions to exactly match the blue pyramid in the image"""
        center_row = self.grid_size // 2 - 1
        center_col = self.grid_size // 2 - 1
        ...
        positions.append((center_row - 1, center_col))
        positions.append((center_row - 1, center_col + 1))
        positions.append((center_row - 1, center_col + 2))
        ...
        # Return exactly 18 cells (or fewer if that's all we have)
        return positions[:18]

    def get_diamond_shape(self):
        """Generate diamond shape positions (exactly 20 cells) with better balance"""
        center_row = self.grid_size // 2 - 1
        center_col = self.grid_size // 2 - 1
        ...
        # Middle row - 5 cells (widest part)
        for c in range(center_col - 2, center_col + 3):
            positions.append((center_row - 1, c))
        ...
        # Add a few more cells to make it more balanced if needed
        if len(positions) < 20:
            # Add cells to the sides of the middle row to make it wider
            positions.append((center_row - 1, center_col - 3))
            positions.append((center_row - 1, center_col + 3))

            # Add cells to the second and fourth rows to make them wider
            positions.append((center_row - 2, center_col - 2))
            positions.append((center_row - 2, center_col + 2))
            positions.append((center_row, center_col - 2))
            positions.append((center_row, center_col + 2))
    ...
    def get_circle_shape(self):
        ...
        positions.sort(key=lambda pos: abs(
            (pos[0] - center_row)**2 + (pos[1] - center_col)**2 - radius**2))

        return positions[:20]

    def get_ring_shape(self):
        """Generate ring shape positions (hollow circle, exactly 20 cells)"""
        center_row = self.grid_size // 2 - 1
        center_col = self.grid_size // 2 - 1

        positions = []

        # Define the ring with inner and outer radius
        outer_radius = 2.5
        inner_radius = 1.0
        ...
        for r in range(center_row - 3, center_row + 4):
            for c in range(center_col - 3, center_col + 4):
                ...
                # Include points between inner and outer radius
                if inner_radius <= distance <= outer_radius:
                    positions.append((r, c))

        # Sort positions by how close they are to the ideal ring
        positions.sort(key=lambda pos: abs(
            math.sqrt((pos[0] - center_row)**2 + (pos[1] - center_col)**2) - ((inner_radius + outer_radius) / 2)))

        return positions[:20]

    def get_exact_pyramid_shape(self):
        """Generate pyramid shape positions exactly matching the image (exactly 20 cells)"""
        center_row = self.grid_size // 2 - 1
        center_col = self.grid_size // 2 - 1
        ...
        # Add more cells if needed to reach 20
        if len(positions) < 20:
            # Add cells to the sides of the bottom row
            positions.append((center_row + 1, center_col - 4))
            positions.append((center_row + 1, center_col + 4))

            # Add cells to the sides of the middle row
            positions.append((center_row, center_col - 3))
            positions.append((center_row, center_col + 3))

        return positions[:20]

    def get_square_shape(self):
        """Generate square shape positions exactly matching the image (exactly 20 cells)"""
        center_row = self.grid_size // 2 - 1
        center_col = self.grid_size // 2 - 1

        positions = []

        # Create a 5x5 square with two cells missing in the top-left corner
        for r in range(center_row - 2, center_row + 3):
            for c in range(center_col - 2, center_col + 3):
                # Skip the two cells in the top-left corner (green cells in the image)
                if not (r == center_row - 2 and c == center_col - 2) and not (r == center_row - 2 and c == center_col - 1):
                    positions.append((r, c))

        # Ensure we have exactly 20 cells (5x5 - 2 = 23 cells)
        return positions[:20]
    ...
    def open_shape_designer(self):
        ...
        if self.movement_in_progress:
            self.status_var.set(
                "Cannot design shape while movement is in progress")
            return

        ShapeDesignerWindow(self.root, self.grid_size,
                            self.on_custom_shape_created)
    ...
    def on_custom_shape_created(self, shape_cells):
        ...
        if len(shape_cells) > 20:
            ...
            candidates.sort(key=lambda pos: abs(
                pos[0] - center_row) + abs(pos[1] - center_col))
        ...
        self.status_var.set(
            f"Custom shape created with {len(shape_cells)} cells")
        self.update_target_highlights()
    ...
    def generate_random_obstacles(self):
        ...
        occupied = set(pos for pos, cell in self.cells.items()
                       if cell["active"])

        total_cells = self.grid_size * self.grid_size
        num_obstacles = random.randint(
            int(total_cells * 0.05), int(total_cells * 0.15))
        ...
        while obstacle_count < num_obstacles and attempts < max_attempts:
            ...
            self.canvas.itemconfig(
                self.cells[pos]["rect"], fill=OBSTACLE_COLOR)
            obstacle_count += 1
        ...
        if 0 <= row < self.grid_size and 0 <= col < self.grid_size:
            ...
            if self.obstacle_mode:
                ...
                if self.cells[pos]["active"]:
                    self.status_var.set(
                        "Cannot place obstacles on active cells")
                    return
        ...
        for row in range(self.grid_size):
            for col in range(self.grid_size):
                ...
                rect = self.canvas.create_rectangle(
                    x1, y1, x2, y2, fill=INACTIVE_COLOR, outline=GRID_COLOR)
        ...
        for row in range(self.grid_size - 1, -1, -1):
            for col in range(self.grid_size - 1, -1, -1):
                if active_count < self.num_active_cells:
                    ...
                    self.canvas.itemconfig(
                        self.cells[pos]["rect"], fill=ACTIVE_COLOR)
                    self.cell_numbers[pos] = self.next_cell_number
                    self.canvas.itemconfig(
                        self.cell_number_text[pos], text=str(self.next_cell_number))
                    self.next_cell_number += 1
                    active_count += 1
    ...
    def get_directions(self):
        """Get movement directions including diagonal movement"""
        # Always return all 8 directions (4 cardinal + 4 diagonal)
    ...
    def find_alternate_path(self, start_pos, target_pos, occupied_positions, obstacles):
        ...
        grid = [[0 for _ in range(self.grid_size)]
    ...
    def is_move_valid_with_cohesion(self, start_pos, next_pos, occupied_positions):
        ...
        occupied_after_move = set(
            pos for pos in occupied_positions if pos != start_pos)
    ...
    def reset_grid(self):
        ...
        active_cells = [pos for pos, cell in self.cells.items()
                        if cell["active"]]
        ...
        if len(cells_to_assign) > len(target_positions):
            self.status_var.set(
                "Error: More active cells than target positions")
            self.movement_in_progress = False
        ...
        self.status_var.set(
            f"Moving {len(self.active_to_target_assignments)} cells back to starting position...")
    ...
    def move_cells_to_reset(self):
        ...
        occupied_positions = set(
            pos for pos, cell in self.cells.items() if cell["active"])
    ...
    def update_counter(self):
        ...
        self.counter_var.set(
            f"Active cells: {active_count}/{self.num_active_cells}")
    ...
    def form_shape(self):
        ...
        if len(active_cells) != self.num_active_cells:
            self.status_var.set(
                f"Need exactly {self.num_active_cells} active cells. Current count: {len(active_cells)}")
            return
        ...
        obstacle_targets = [
            pos for pos in target_positions if pos in self.obstacles]
        if obstacle_targets:
            self.status_var.set(
                f"Error: {len(obstacle_targets)} target positions overlap with obstacles")
            self.movement_in_progress = False
            self.form_button.config(state=tk.NORMAL)
            return
        ...
        for target in ordered_targets:
            ...
            closest_cell = min(
                remaining_cells, key=lambda cell: self.manhattan_dist(cell, target))
            assignments[closest_cell] = target
            remaining_cells.remove(closest_cell)

        self.active_to_target_assignments = assignments
        self.status_var.set(
            f"Starting formation with {len(self.active_to_target_assignments)} cells to move...")
        self.move_cells()
    ...
    def prioritize_targets(self, targets):
        ...
        if self.selected_shape.get() == "rectangle":
            ...
            def diamond_priority(pos):
                # Calculate distance from center
                dist_from_center = abs(
                    pos[0] - center_row) + abs(pos[1] - center_col)

                # Prioritize middle row first, then work outward
                if pos[0] == center_row - 1:  # Middle row (widest part)
                    row_priority = 0
                elif pos[0] == center_row - 2:  # Second row
                    row_priority = 1
                elif pos[0] == center_row:  # Fourth row
                    row_priority = 2
                elif pos[0] == center_row - 3:  # Top point
                    row_priority = 3
                elif pos[0] == center_row + 1:  # Bottom point
                    row_priority = 4
                else:
                    row_priority = 5

                # Within each row, prioritize center columns first
                col_priority = abs(pos[1] - center_col)

                return (row_priority, col_priority)

            return sorted(targets, key=diamond_priority)

        elif self.selected_shape.get() in ["heart", "arrow"]:
            center_row = self.grid_size // 2 - 1
            center_col = self.grid_size // 2 - 1
            ...
            def cross_priority(pos):
                ...
                center_dist = abs(pos[0] - center_row) + \
                    abs(pos[1] - center_col)
        ...
        for stuck_pos in list(self.cells_requesting_help):
            ...
            for target_pos in available_targets:
                ...
                next_step = self.find_next_step(
                    stuck_pos, target_pos, occupied_positions)
                if next_step:
    ...
    def reassign_all_targets(self):
        """Completely reassign all targets to break severe deadlocks"""
        active_cells = [pos for pos, cell in self.cells.items()
                        if cell["active"]]
        all_targets = self.get_target_positions()
        incomplete_targets = [
            t for t in all_targets if t not in self.completed_targets]

        if len(active_cells) == 0 or len(incomplete_targets) == 0:
            return

        print(
            f"Completely reassigning all targets for {len(active_cells)} cells")

        # Clear current assignments
        self.active_to_target_assignments = {}

        # Sort cells by their distance to the center
        center_row = self.grid_size // 2 - 1
        center_col = self.grid_size // 2 - 1
        center = (center_row, center_col)

        active_cells.sort(key=lambda pos: self.manhattan_dist(pos, center))

        # Sort targets by priority
        ordered_targets = self.prioritize_targets(incomplete_targets)

        # Assign targets to cells
        for i, cell_pos in enumerate(active_cells):
            if i < len(ordered_targets):
                self.active_to_target_assignments[cell_pos] = ordered_targets[i]

        print(f"Reassigned {len(self.active_to_target_assignments)} targets")

    def try_spread_cells(self):
        """Try to move cells away from each other to reduce congestion"""
        active_cells = [pos for pos, cell in self.cells.items()
                        if cell["active"]]
        ...
        # Find cells that are too close to each other
        congested_cells = []
        for i, pos1 in enumerate(active_cells):
            for pos2 in active_cells[i+1:]:
                if self.manhattan_dist(pos1, pos2) == 1:
                    if pos1 not in congested_cells:
                        congested_cells.append(pos1)
                    if pos2 not in congested_cells:
                        congested_cells.append(pos2)

        if not congested_cells:
            return False

        print(f"Found {len(congested_cells)} congested cells")

        # Try to move congested cells away from each other
        cells_to_move = {}
        for pos in congested_cells:
            # Find all neighboring cells
            neighbors = []
            for other_pos in active_cells:
                if other_pos != pos and self.manhattan_dist(pos, other_pos) == 1:
                    neighbors.append(other_pos)

            if not neighbors:
                continue

            # Find possible moves away from neighbors
            possible_moves = []
            for dr, dc in self.get_directions():
                ...
                if (0 <= new_pos[0] < self.grid_size and
                    ...
                    new_pos not in active_cells and
                        new_pos not in self.obstacles):

                    # Calculate how much this move reduces congestion
                    current_congestion = sum(
                        self.manhattan_dist(pos, n) for n in neighbors)
                    new_congestion = sum(self.manhattan_dist(
                        new_pos, n) for n in neighbors)

                    if new_congestion > current_congestion:
                        possible_moves.append(
                            (new_pos, new_congestion - current_congestion))

            if possible_moves:
                # Choose the move that reduces congestion the most
                best_move = max(possible_moves, key=lambda x: x[1])
                cells_to_move[pos] = best_move[0]
    ...
    def move_random_cells(self):
        """Move random cells to random valid positions to break severe deadlocks"""
        active_cells = [pos for pos, cell in self.cells.items()
                        if cell["active"]]
        ...
        # Randomly select up to 3 cells to move
        num_cells_to_move = min(3, len(active_cells))
        cells_to_move = random.sample(active_cells, num_cells_to_move)
        ...
        for pos in cells_to_move:
            ...
            if valid_moves:
                ...
                moves[pos] = random.choice(valid_moves)
    ...
    def update_help_requests(self):
        ...
        for start_pos, target_pos in self.active_to_target_assignments.items():
            ...
            path = self.find_next_step(
                start_pos, target_pos, occupied_positions)
            ...
            if path is None:
                ...
                if self.stuck_cell_history[start_pos] >= 3:
                    ...
                    self.canvas.itemconfig(
                        self.cells[start_pos]["rect"], fill=HELP_REQUEST_COLOR)
            else:
    ...
    def remove_obstacles_near_stuck_cells(self):
        """Remove obstacles that are causing deadlocks"""
        # Find stuck cells
        stuck_cells = set()

        # First, identify cells that can't reach their targets
        for start_pos, target_pos in self.active_to_target_assignments.items():
            occupied_positions = set(
                pos for pos, cell in self.cells.items() if cell["active"])
            path = self.find_next_step(
                start_pos, target_pos, occupied_positions)
            if path is None:
                stuck_cells.add(start_pos)

        if not stuck_cells:
            # If no cells are explicitly stuck, use cells requesting help
            stuck_cells = self.cells_requesting_help.copy()

        if not stuck_cells:
            # If still no stuck cells, find cells with the most obstacles around them
            active_cells = [pos for pos,
                            cell in self.cells.items() if cell["active"]]
            cells_with_obstacles = []
            ...
            for pos in active_cells:
                ...
                if obstacle_count > 0:
                    cells_with_obstacles.append((pos, obstacle_count))

            # Sort by obstacle count (descending)
            cells_with_obstacles.sort(key=lambda x: -x[1])

            # Take the top 3 cells with the most obstacles
            for pos, _ in cells_with_obstacles[:3]:
                stuck_cells.add(pos)

        # Now remove obstacles near stuck cells
        obstacles_to_remove = set()

        for stuck_pos in stuck_cells:
            # Find obstacles within 2 cells of the stuck cell
            for r in range(max(0, stuck_pos[0] - 2), min(self.grid_size, stuck_pos[0] + 3)):
                for c in range(max(0, stuck_pos[1] - 2), min(self.grid_size, stuck_pos[1] + 3)):
                    pos = (r, c)
                    if pos in self.obstacles:
                        obstacles_to_remove.add(pos)
        ...
        self.status_var.set(
            f"Removed {len(obstacles_to_remove)} obstacles to resolve deadlock")
        self.update_target_highlights()
    ...
    def move_cells_sequential(self, occupied_positions, for_reset=False):
        ...
        if self.coordination_mode.get() and not for_reset:
            ...
            for helper_pos, target_cells in self.help_response_cells.items():
                ...
                next_step = self.find_clearing_move(
                    helper_pos, occupied_positions)
                if next_step:
        ...
        if not cells_to_move:
            ...
            for start_pos, target_pos in list(self.active_to_target_assignments.items()):
                ...
                if next_step is None and for_reset:
                    next_step = self.find_alternate_path(
                        start_pos, target_pos, occupied_positions, self.obstacles)
                ...
                if next_step:
                    ...
                    if for_reset:
                        ...
                        if self.selected_shape.get() == "rectangle":
                            ...
                            center_col = self.grid_size // 2 - 1
                            priority = abs(
                                target_pos[0] - center_row) + abs(target_pos[1] - center_col)
        ...
        if cells_to_move:
            ...
            if for_reset:
                self.pending_callback = self.root.after(
                    self.speed_var.get(), self.move_cells_to_reset)
            else:
                self.pending_callback = self.root.after(
                    self.speed_var.get(), self.move_cells)
        else:
            if for_reset:
    ...
    def find_clearing_move(self, cell_pos, occupied_positions):
        ...
        for dr, dc in directions:
            ...
            if helped_cells > 0:
                ...
                own_benefit = self.manhattan_dist(
                    cell_pos, target_pos) - self.manhattan_dist(neighbor, target_pos)
    ...
    def move_cells_parallel(self, occupied_positions, for_reset=False):
        ...
        self.resolve_conflicts_with_priority(
            cells_to_move, for_reset=for_reset)
    ...
    def move_cells_async(self, occupied_positions, for_reset=False):
        ...
        if cells_to_move:
            ...
            max_moves = min(len(cells_to_move), max(
                1, len(cells_to_move) // 2))
            ...
            filtered_moves = {pos: cells_to_move[pos]
                              for pos in selected_cells}

            self.resolve_conflicts_with_priority(
                filtered_moves, for_reset=for_reset)
            ...
            if filtered_moves:
                ...
                if for_reset:
                    self.pending_callback = self.root.after(
                        delay, self.move_cells_to_reset)
                else:
                    self.pending_callback = self.root.after(
                        delay, self.move_cells)
            else:
                if for_reset:
                    self.handle_reset_deadlock()
                else:
    ...
    def execute_moves(self, cells_to_move):
        ...
        for start_pos, next_pos in cells_to_move.items():
            ...
            self.canvas.itemconfig(
                self.cells[start_pos]["rect"], fill=INACTIVE_COLOR)
            self.cells[next_pos]["active"] = True
            self.canvas.itemconfig(
                self.cells[next_pos]["rect"], fill=ACTIVE_COLOR)

            if start_pos in self.cell_numbers:
                cell_num = self.cell_numbers[start_pos]
                self.canvas.itemconfig(
                    self.cell_number_text[start_pos], text="")
                self.cell_numbers[next_pos] = cell_num
                self.canvas.itemconfig(
                    self.cell_number_text[next_pos], text=str(cell_num))
        ...
        self.status_var.set(
            f"Moving cells ({mode_str}, {algo_str}, {coord_str}, {cohesion_str}). {remaining} left to reach targets. {completed} in position.")
        self.update_counter()
        self.update_metrics_display()
    ...
    def resolve_conflicts_with_priority(self, cells_to_move, for_reset=False):
        ...
        for dest, sources in destination_counts.items():
            if len(sources) > 1:
                ...
                helper_sources = [
                    s for s in sources if s in self.help_response_cells]
                        ...
                        for source in sources:
                            if source in self.active_to_target_assignments:
                                ...
                                if self.selected_shape.get() in ["rectangle", "pyramid", "diamond", "circle", "cross", "heart", "arrow"]:
                                    ...
                                    center_col = self.grid_size // 2 - 1
                                    priority = abs(
                                        target[0] - center_row) + abs(target[1] - center_col)
                                elif self.selected_shape.get() == "custom" and self.custom_shape:

                                    try:
                                        priority = self.custom_shape.index(
                                            target)
                                    except ValueError:
                                        center_row = self.grid_size // 2 - 1
                                        center_col = self.grid_size // 2 - 1
                ...
                if prioritized_source is None:
                    prioritized_source = min(sources, key=lambda s:
                                             self.manhattan_dist(
                                                 s, self.active_to_target_assignments[s])
    ...
    def find_next_step_astar(self, start_pos, target_pos, occupied_positions):
        """Improved A* Search Algorithm with better obstacle avoidance"""
        occupied = {pos for pos in occupied_positions if pos !=
                    start_pos} | self.obstacles
        ...
        came_from = {}
        g_score = {start_pos: 0}
        f_score = {start_pos: self.manhattan_dist(
            start_pos, target_pos) * self.heuristic_weight.get()}
        closed_set = set()
        counter = 1
        max_iterations = self.grid_size * self.grid_size * 4  # Increased max iterations
        ...
        while open_list and iterations < max_iterations:
            ...
            # Sort directions to prioritize moves toward the target
            directions = self.get_directions()
            ...
            for dr, dc in directions:
                ...
                for obs_pos in self.obstacles:
                    ...
                    if dist_to_obstacle <= 2:
                        # Higher penalty for very close obstacles
                ...
                tentative_g_score = g_score[current] + \
                    move_cost + obstacle_penalty

                if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                    came_from[neighbor] = current
                    g_score[neighbor] = tentative_g_score
                    h_score = self.manhattan_dist(
                        neighbor, target_pos) * self.heuristic_weight.get()

                    obstacle_penalty = 0
                    for obs_pos in self.obstacles:
                        dist_to_obstacle = self.manhattan_dist(
                            neighbor, obs_pos)
                        if dist_to_obstacle <= 2:
        ...
        while open_list and iterations < max_iterations:
            ...
            for dr, dc in directions:
                ...
                if neighbor not in came_from:
                    ...
                    heappush(
                        open_list, (h_score + obstacle_penalty, counter, neighbor))
                    counter += 1
    ...
    def handle_deadlock(self):
        ...
        if self.selected_shape.get() == "diamond" and self.try_diamond_specific_resolution():
            self.status_var.set(
                "Using diamond-specific resolution strategy...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells)
            return

        # Try more aggressive target reassignment first
        if self.deadlock_count >= 2:
            print("Trying aggressive target reassignment")
            self.reassign_all_targets()
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells)
            return
        ...
        if self.coordination_mode.get():
            ...
            if self.help_response_cells:
                self.status_var.set(
                    "Using coordinated movement to resolve deadlock...")
                self.pending_callback = self.root.after(
                    self.speed_var.get(), self.move_cells)
                return

        # Try to move cells away from each other if they're too close
        if self.try_spread_cells():
            self.status_var.set("Spreading cells to avoid congestion...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells)
            return
        ...
        if self.move_any_cell_to_free_space():
            self.status_var.set(
                "Moving random cell to free space to break deadlock...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells)
            return
        ...
        if self.coordination_mode.get() and self.try_multi_cell_movement():
            self.status_var.set(
                "Coordinating multi-cell movement to resolve deadlock...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells)
            return

        # More aggressive approach for persistent deadlocks
        if self.deadlock_count >= 3 and self.deadlock_count % 2 == 1:
            print("Trying random movement to break severe deadlock")
            self.move_random_cells()
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells)
            return

        if self.deadlock_count < 10:  # Increased from 5 to 10 to give more attempts
            self.status_var.set(
                f"Deadlock persists, trying again (attempt {self.deadlock_count})...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells)
        else:
            self.status_var.set(
                "Cannot complete formation - deadlock detected.")
    ...
    def handle_reset_deadlock(self):
        ...
        if self.try_temporary_moves(for_reset=True):
            self.status_var.set(
                "Making temporary moves to resolve deadlock...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells_to_reset)
            return
        ...
        if self.try_obstacle_path_clearing(for_reset=True):
            self.status_var.set(
                "Clearing path around obstacles during reset...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells_to_reset)
            return
        ...
        if self.try_multi_cell_movement(for_reset=True):
            self.status_var.set(
                "Attempting coordinated multi-cell movement...")
            self.pending_callback = self.root.after(
                self.speed_var.get(), self.move_cells_to_reset)
            return

        self.status_var.set(
            "Unable to complete animated reset. Performing quick reset.")
        ...
        for pos, _, _ in priority_cells[:5]:
            ...
            for empty_pos in empty_spaces:
                ...
                if is_adjacent:
                    ...
                    self.canvas.itemconfig(
                        self.cells[empty_pos]["rect"], fill=ACTIVE_COLOR)

                    if pos in self.cell_numbers:
                        cell_num = self.cell_numbers[pos]
                        self.canvas.itemconfig(
                            self.cell_number_text[pos], text="")
                        self.cell_numbers[empty_pos] = cell_num
                        self.canvas.itemconfig(
                            self.cell_number_text[empty_pos], text=str(cell_num))
    ...
    def try_diamond_specific_resolution(self):
        """Special deadlock resolution strategy for diamond shape"""
        if self.selected_shape.get() != "diamond":
            return False

        print("Applying diamond-specific deadlock resolution")

        # Get active cells and their targets
        active_cells = [pos for pos, cell in self.cells.items()
                        if cell["active"]]
        if not active_cells:
            return False
        ...
        # Identify cells in the middle row (the widest part)
        middle_row_cells = [
            pos for pos in active_cells if pos[0] == center_row - 1]

        # Identify cells that are stuck in the middle
        stuck_cells = []
        for pos in active_cells:
            if pos in self.active_to_target_assignments:
                target = self.active_to_target_assignments[pos]
                # Check if cell is trying to move to the middle row
                if target[0] == center_row - 1:
                    # Check if it's blocked
                    next_step = self.find_next_step(
                        pos, target, set(active_cells) - {pos})
                    if not next_step:
                        stuck_cells.append(pos)

        if not stuck_cells:
            return False

        print(
            f"Found {len(stuck_cells)} cells stuck trying to reach the middle row")

        # Try to move cells out of the way
        moves = {}

        # First, try to move cells in the middle row that might be blocking others
        for pos in middle_row_cells:
            # Find valid moves that would clear space
            valid_moves = []
            for dr, dc in self.get_directions():
                new_pos = (pos[0] + dr, pos[1] + dc)

                # Check if move is valid and would help clear congestion
                if (0 <= new_pos[0] < self.grid_size and
                    0 <= new_pos[1] < self.grid_size and
                    new_pos not in active_cells and
                        new_pos not in self.obstacles):

                    # Prioritize moves that help form the diamond shape
                    if pos in self.active_to_target_assignments:
                        target = self.active_to_target_assignments[pos]
                        if self.manhattan_dist(new_pos, target) < self.manhattan_dist(pos, target):
                            # Higher priority for moves toward target
                            valid_moves.append((new_pos, 2))
                        else:
                            # Lower priority for other moves
                            valid_moves.append((new_pos, 1))

            if valid_moves:
                # Choose the highest priority move
                best_move = max(valid_moves, key=lambda x: x[1])
                moves[pos] = best_move[0]

        # If no middle row cells can move, try moving stuck cells
        if not moves:
            for pos in stuck_cells:
                # Find any valid move
                valid_moves = []
                for dr, dc in self.get_directions():
                    new_pos = (pos[0] + dr, pos[1] + dc)
        ...
        # If still no moves, try a more drastic approach: temporarily reassign targets
        if not moves:
            print("Temporarily reassigning targets for diamond shape")

            # Get all incomplete targets
            all_targets = self.get_target_positions()
            incomplete_targets = [
                t for t in all_targets if t not in self.completed_targets]

            # Prioritize targets by row
            def diamond_row_priority(pos):
                if pos[0] == center_row - 1:  # Middle row
                    return 0
                elif pos[0] == center_row - 2:  # Second row
                    return 1
                elif pos[0] == center_row:  # Fourth row
            ...
            prioritized_targets = sorted(
                incomplete_targets, key=diamond_row_priority)

            # Reassign targets for stuck cells
            for i, pos in enumerate(stuck_cells):
                if i < len(prioritized_targets):
                    self.active_to_target_assignments[pos] = prioritized_targets[i]
        ...
        for pos, target in active_with_targets:
            if target:
                ...
                for dr, dc in directions:
                    ...
                    occupied_positions = set(
                        p for p, cell in self.cells.items() if cell["active"])
                    ...
                    if new_dist < current_dist or (new_dist == current_dist and self.temp_move_count > 5):
                        ...
                        self.canvas.itemconfig(
                            self.cells[neighbor]["rect"], fill=ACTIVE_COLOR)
                        ...
                        if pos in self.cell_numbers:
                            ...
                            self.canvas.itemconfig(
                                self.cell_number_text[neighbor], text=str(cell_num))
                            del self.cell_numbers[pos]
    ...
    def move_blocking_cell(self, blocking_pos):
        ...
        if blocking_pos in self.active_to_target_assignments:
            ...
            empty_neighbors.sort(
                key=lambda pos: self.manhattan_dist(pos, target))

        next_pos = empty_neighbors[0]

        self.cells[blocking_pos]["active"] = False
        self.canvas.itemconfig(
            self.cells[blocking_pos]["rect"], fill=INACTIVE_COLOR)
        self.cells[next_pos]["active"] = True
        ...
        if blocking_pos in self.cell_numbers:
            ...
            self.canvas.itemconfig(
                self.cell_number_text[blocking_pos], text="")
            self.cell_numbers[next_pos] = cell_num
            self.canvas.itemconfig(
                self.cell_number_text[next_pos], text=str(cell_num))
                ...
                for empty_pos in empty_spaces:
                    ...
                    for dr, dc in directions:
                        if (cell_pos[0] + dr, cell_pos[1] + dc) == empty_pos:
                            ...
                            self.canvas.itemconfig(
                                self.cells[cell_pos]["rect"], fill=INACTIVE_COLOR)
                            self.cells[empty_pos]["active"] = True
                            self.canvas.itemconfig(
                                self.cells[empty_pos]["rect"], fill=ACTIVE_COLOR)
                            ...
                            if cell_pos in self.cell_numbers:
                                ...
                                self.canvas.itemconfig(
                                    self.cell_number_text[cell_pos], text="")
                                self.cell_numbers[empty_pos] = cell_num
                                self.canvas.itemconfig(
                                    self.cell_number_text[empty_pos], text=str(cell_num))
...
class ShapeDesignerWindow:
    ...
    def __init__(self, parent, grid_size, on_shape_created):
        ...
        tk.Label(control_frame, text=f"Select exactly 20 cells for your custom shape").pack(
        ...
        self.save_button = tk.Button(
            button_frame, text="Save Shape", command=self.save_shape)
        ...
        tk.Label(template_frame, text="Quick Templates:").pack(
            side=tk.LEFT, padx=5)
        ...
        template_buttons = [
            ...
            ("Pyramid", self.create_pyramid_template)
        ]

        for text, command in template_buttons:
            tk.Button(template_frame, text=text, command=command).pack(
                side=tk.LEFT, padx=5)
    ...
    def on_canvas_drag(self, event):
        ...
        if 0 <= row < self.grid_size and 0 <= col < self.grid_size:
            ...
            if pos not in self.selected_cells:
                ...
                self.canvas.itemconfig(self.cells[pos], fill="blue")
                self.counter_var.set(
                    f"Selected: {len(self.selected_cells)}/20")
    ...
    def save_shape(self):
        if len(self.selected_cells) != 20:
            messagebox.showwarning(
                "Invalid Shape", f"Please select exactly 20 cells. Current count: {len(self.selected_cells)}")
            return
    ...
    def create_diamond_template(self):
        ...
        # Top point
        diamond_shape.append((center_row - 3, center_col))

        # Second row - 3 cells
        diamond_shape.append((center_row - 2, center_col - 1))
        diamond_shape.append((center_row - 2, center_col))
        diamond_shape.append((center_row - 2, center_col + 1))
        ...
        # Fourth row - 3 cells
        diamond_shape.append((center_row, center_col - 1))
        diamond_shape.append((center_row, center_col))
        diamond_shape.append((center_row, center_col + 1))

        # Bottom point
        diamond_shape.append((center_row + 1, center_col))

        # Add a few more cells to make it more balanced if needed
        diamond_shape.append((center_row - 1, center_col - 3))
        diamond_shape.append((center_row - 1, center_col + 3))
        diamond_shape.append((center_row - 2, center_col - 2))
        diamond_shape.append((center_row - 2, center_col + 2))
        diamond_shape.append((center_row, center_col - 2))
        diamond_shape.append((center_row, center_col + 2))
    ...
    def create_ring_template(self):
        """Create a ring-shaped template (hollow circle)"""
        self.clear_all()
    ...
    def create_pyramid_template(self):
        """Create a pyramid-shaped template matching the image"""
        self.clear_all()
        ...
        # Bottom row (7 cells)
        for c in range(center_col - 3, center_col + 4):
            pyramid_shape.append((center_row + 1, c))

        # Middle row (5 cells)
        for c in range(center_col - 2, center_col + 3):
            pyramid_shape.append((center_row, c))
        ...
        # Top row (1 cell)
        pyramid_shape.append((center_row - 2, center_col))

        # Add more cells if needed to reach 20
        if len(pyramid_shape) < 20:
            # Add cells to the sides of the bottom row
            pyramid_shape.append((center_row + 1, center_col - 4))
            pyramid_shape.append((center_row + 1, center_col + 4))

            # Add cells to the sides of the middle row
            pyramid_shape.append((center_row, center_col - 3))
            pyramid_shape.append((center_row, center_col + 3))

        for pos in pyramid_shape[:20]:
            if 0 <= pos[0] < self.grid_size and 0 <= pos[1] < self.grid_size:
                self.selected_cells.add(pos)
                self.canvas.itemconfig(self.cells[pos], fill="blue")
    ...
    def create_square_template(self):
        """Create a square-shaped template matching the image"""
        self.clear_all()
        ...
        for r in range(center_row - 2, center_row + 3):
            for c in range(center_col - 2, center_col + 3):
                # Skip the two cells in the top-left corner (green cells in the image)
                if not (r == center_row - 2 and c == center_col - 2) and not (r == center_row - 2 and c == center_col - 1):
                    square_shape.append((r, c))

        for pos in square_shape[:20]:
            if 0 <= pos[0] < self.grid_size and 0 <= pos[1] < self.grid_size:
                self.selected_cells.add(pos)
                self.canvas.itemconfig(self.cells[pos], fill="blue")
